...
public partial class Vote_IndexVoteShow : System.Web.UI.Page
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            if (!string.IsNullOrEmpty(Request.QueryString["id"]))
            {
                ...
                voteMark = string.IsNullOrEmpty(tt.vote_mark.Trim()) == true ? "" : "描述:" + tt.vote_mark;
                PageBind();
            }
        }
    }
    ...
    public void PageBind()
    {
        ...
        if (tvv != null)
        {
            ...
            if (tviiList.Count > 0)
            {
                for (int i = 0; i < tviiList.Count; i++)
                {
                    allCount += Convert.ToInt32(tviiList[i].ivote_Count.ToString());
                }
